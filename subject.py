...
class Ui_MainWindow(object):
    ...
    def LoadData(self):
        ...
        cursor = con.cursor()
        cursor.execute("SELECT `subject_name`, `departament_id` FROM `subject` WHERE 1")
    ...
    def InsertData(self):
        ...
        global ids
        subname = self.lineEdit.text()
        if (subname==''):
            print("empty")
        else:
            cursor = con.cursor()
            cursor.execute("INSERT INTO subject(subject_name,departament_id)"
                                "VALUES('%s','%s')" % (''.join(subname),
                                                       ''.join(ids),
                                                      ))
            con.commit()
        #self.close()
    def setupUi(self, MainWindow):
        ...
        self.tableWidget.setHorizontalHeaderLabels(["Название дисцилпины", "Номер кафедры"])
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.lineEdit.setPlaceholderText(_translate("MainWindow", "Название дисциплины"))
